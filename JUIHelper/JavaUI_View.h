...
#ifndef __NDKGUI_JAVAUI_VIEW_H__
#define __NDKGUI_JAVAUI_VIEW_H__
...
public:
    ...
    if (it != map.end()) {
      ...
      switch (it->second) {
        case ATTRIBUTE_PARAMETER_INT:
          JNIHelper::GetInstance()->CallVoidMethod(
              obj_, s.c_str(), "(I)V", t);
    ...
    if (it != map.end()) {
      ...
      switch (it->second) {
        case ATTRIBUTE_PARAMETER_STRING: {
          JNIEnv *env =
              JNIHelper::GetInstance()->AttachCurrentThread();
  ...
  bool GetAttribute(std::unordered_map<std::string, int32_t> &map,
    ...
    if (it != map.end()) {
      ...
      switch (it->second) {
        case ATTRIBUTE_PARAMETER_INT:
          ret = (T)JNIHelper::GetInstance()->CallIntMethod(
              obj_, s.c_str(), "()I");
          break;
        case ATTRIBUTE_PARAMETER_FLOAT:
          ret = (T)JNIHelper::GetInstance()->CallFloatMethod(
              obj_, s.c_str(), "()F");
          break;
        case ATTRIBUTE_PARAMETER_BOOLEAN:
          ret = (T)JNIHelper::GetInstance()->CallBooleanMethod(
              obj_, s.c_str(), "()Z");
          break;
        default:
          ret = 0;
...
#endif  // __NDKGUI_JAVAUI_VIEW_H__
